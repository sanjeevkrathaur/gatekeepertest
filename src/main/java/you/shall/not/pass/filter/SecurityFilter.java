...
import you.shall.not.pass.exception.NoAccessException;
...
public class SecurityFilter implements Filter {
    ...
    public static final String HIGH_LEVEL_URL = "highLevelUrl";
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        try {
            ...
            ((HttpServletRequest) request).getSession().setAttribute(HIGH_LEVEL_URL, ((HttpServletRequest) request).getRequestURI());
            ...
            LOG.warn("No Access violation, {}", nae.getMessage());
            processNoAccessError((HttpServletResponse) response, nae);
        }
    }

    private void processNoAccessError(HttpServletResponse response, NoAccessException nae) {
        Violation violation = Violation.builder()
                .message(nae.getMessage())
                .csrfPassed(false)
                .build();

        writeResponse(response, violation, HttpStatus.NOT_FOUND);
    }
...
//        response.setStatus(HttpStatus.BAD_REQUEST.value());
        writeResponse(response, violation, HttpStatus.BAD_REQUEST);
    }
...
//        response.setStatus(HttpStatus.FORBIDDEN.value());
        writeResponse(response, violation, HttpStatus.FORBIDDEN);
    }
    ...
    private void shallNotPassLogic(HttpServletRequest request) {
        ...
        resourceValidator.ifPresent(validator -> {
            ...
            if (validator.requires().equals(Access.NoAccessLevel))
                throw new NoAccessException(validator.requires(), "no access request");
    ...
    private void writeResponse(HttpServletResponse response, Violation violation, HttpStatus status) {
        try {
            String message = gson.toJson(violation);
            response.setStatus(status.value());
            switch (status) {
                case FORBIDDEN:
                    response.sendRedirect("/login?message=" + violation.getMessage());
                    break;
                case NOT_FOUND:
                    response.sendRedirect("/resources?message=" + violation.getMessage());
                    break;
                default:
